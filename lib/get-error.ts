...
const errors = {
  cmd: (command: Command) => ({
    ...
    details: `The [\`cmd\` option](${linkify(`README.md#options`)}) is required and must be a non empty \`String\`.

Your configuration for the \`cmd\` option is \`${stringify(command.template)}\`.`,
  }),
  cwd: (command: Command) => ({
    message: "Invalid `cwd` option.",
    details: `The [\`cwd\` option](${linkify("README.md#options")}) if defined, must be a non empty \`String\`.

Your configuration for the \`cwd\` option is \`${stringify(command.cwd)}\`.`,
  }),
};

export function getError(commandKey: string, command: Command): SemanticReleaseError {
